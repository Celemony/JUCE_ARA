...
The develop branch of this fork has been updated to JUCE 7 accordingly. It now merely enhances the JUCE 7 ARAPluginDemo example with several more complex features which are beyond the scope of a simple example, but will be very useful when debugging ARA host applications. It also adds experimental implementations of draft API from the ARA SDK which will come to JUCE mainline only after they have been finalized in the ARA SDK.
